...
export default function FacultyOverview() {
  const { currentUser, documents, globalSearchQuery } = useAppContext();
  ...
  const dynamicDomainStats = domains.map((domain, index) => ({
    id: `d${index + 1}`,
    name: domain,
    count: facultyDocs.filter(d => d.domain === domain).length
  }));
