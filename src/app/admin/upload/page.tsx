...
export default function AdminUpload() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      let type = "UNKNOWN";
      if (selectedFile.name.toLowerCase().endsWith('.pdf')) type = "PDF";
      else if (selectedFile.name.toLowerCase().match(/\.(jpg|jpeg)$/)) type = "JPG";
      else if (selectedFile.name.toLowerCase().endsWith('.png')) type = "PNG";
      else if (selectedFile.name.toLowerCase().endsWith('.docx')) type = "DOCX";
      else if (selectedFile.name.toLowerCase().endsWith('.pptx')) type = "PPTX";
      ...
      addDocument({
        ...
        isStarred: false,
        status: "Approved", // Admins upload approved documents directly
        accessList: accessList.length > 0 ? accessList : ["All Faculty"]
      });
