...
import { mockDomainStats } from "@/lib/mock-data";
...
export default function AdminOverview() {
  ...
  const totalBytes = documents.reduce((acc, doc) => {
    if (doc.isDeleted) return acc;
    let sizeStr = doc.size;
  ...
  // We can just keep the percentage relative to 100GB as requested, or not populate it with fake storage numbers. 
  // User said: "Remove the fake storage usage value/data... Keep the existing Storage section/UI if it is part of the design, but do not populate it with fake storage numbers."
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {mockDomainStats.map((stat) => {
            const actualCount = documents.filter(d => d.domain === stat.name && !d.isDeleted).length;
            return <DomainCard key={stat.id} name={stat.name} count={actualCount} />
          })}
        </div>
      </div>
