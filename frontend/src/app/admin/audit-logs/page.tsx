...
function formatDateTime(timeStr: string) {
  try {
    ...
    const parts = dt.toLocaleString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "numeric", hour12: true }).split(", ");
    // Expected parts: ["Sep 3", "2026", "5:28 AM"]
    if (parts.length >= 3) {
              ...
              {auditLogs.map((log) => (
                <tr key={log.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="text-gray-600">{formatDateTime(log.time).date}</div>
                    <div className="font-medium text-foreground">{formatDateTime(log.time).time}</div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-gray-600">
                    192.168.1.{parseInt(String(log.id).substring(0,2), 16) % 255 || 10}
                  </td>
