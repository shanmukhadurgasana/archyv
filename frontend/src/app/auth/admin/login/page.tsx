      ...
      <div className="bg-white rounded-[2rem] p-6 md:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/50">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-bold text-foreground mb-2">
            {is2FA ? "Two-Factor Authentication" : "Welcome back, Admin"}
          </h2>
          <p className="text-gray-500 text-sm">
            {is2FA ? "Enter the 6-digit code from your authenticator app" : "Sign in to access your Archyv workspace"}
          </p>
        ...
        {is2FA ? (
          <form onSubmit={handle2FASubmit} className="space-y-5">
            ...
            <div className="space-y-2 pt-1">
              ...
              <button 
                type="button"
                onClick={() => { setIs2FA(false); setTempToken(""); setTwoFactorToken(""); }}
                className="w-full bg-white border border-gray-200 hover:bg-gray-50 text-foreground font-medium py-2.5 rounded-xl transition-colors shadow-sm"
              >
                Back to Login
              </button>
            </div>
        ...
        <div className="mt-5 text-center text-sm text-gray-500">
          ...
          <Link href="/auth/admin/signup" className="text-[var(--archyv-accent-hover)] font-medium hover:underline">
            Sign up now
          </Link>
        </div>
